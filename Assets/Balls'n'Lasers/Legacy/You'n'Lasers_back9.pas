          ...
          VK_Right:
            begin
              ...
              if not rstop and (Fields[plxr, plyd, 1] = 0) then begin
                ...
                vRight := true; 
                if not LookR and OnFloor then begin LookR := true; LookL := false; end;
                if canmove then begin  if OnFloor then Player.State := 1 else Player.State := 3; end; hspd := hforce; end; end;
          VK_Space: begin vSpase := true;  if not CanMove then exit; if canmove and OnFloor then begin vspd := vforce; if LookR then begin if not vRight then Player.State := 3; end else begin  if not vLeft then Player.State := 6; end; end; end;
...
begin
  ...
  UnLockDrawingObjects;
  //LoadScreen.ToFront;
  for i := 2 to 5 do begin LoadScreen.Frame := i; Sleep(1000); end;
