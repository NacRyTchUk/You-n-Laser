...
begin
  ...
  for ij := 1 to 5 do
    ...
    begin
      ...
      for j := 0 to 20 do
      begin
        if blkInfo[j] <> '' then Fields[j, i, ij, curscreen] := StrToInt(blkInfo[j]);
      end;
    end;
...
begin
  ...
  for ij := 1 to 5 do
    ...
    begin
      Readln(Fieldfile, Line);
      var blkInfo: array of Str := Line.Split(';');
      SetLength(blkInfo, 16);
      for j := 1 to 16 do
      begin
        if blkInfo[j] <> '' then minigameField[j, i] := StrToInt(blkInfo[j]);
      end;
    end;
...
begin
  ...
  if (0  >= px  + hspd) then begin Textout(400, 20, 'left   ');  undrawfield(); Dec(CurScreenX);CurScreen := LocationOfFields[CurScreenx,CurScreenY]; loadscreenfrombuffer(FieldsName[CurScreen-1]); drawfield(1, CurScreen);  Player.Moveto(window.Width - 70, py); end;
  if (px + Player.Width - 10 + hspd >= Window.Width) then begin TextOut(400, 20, 'right  '); 
  undrawfield(); 
  Inc(CurScreenX);
  CurScreen := LocationOfFields[CurScreenx,CurScreenY];
  loadscreenfrombuffer(FieldsName[CurScreen-1]); drawfield(1, CurScreen);  Player.Moveto(8, py); end;
  if (0  >= py - vspd) then begin TextOut(400, 20, 'up    '); undrawfield(); Dec(CurScreenY); CurScreen := LocationOfFields[CurScreenx,CurScreenY]; loadscreenfrombuffer(FieldsName[CurScreen-1]); drawfield(1, CurScreen);  Player.Moveto(px, Window.Height - Player.Height); end;
  if (py - vspd  >= Window.Height) then begin TextOut(400, 20, 'down '); undrawfield(); Inc(CurScreenY); CurScreen := LocationOfFields[CurScreenx,CurScreenY]; loadscreenfrombuffer(FieldsName[CurScreen-1]); drawfield(1, CurScreen);  Player.Moveto(px, player.Height);  end;
