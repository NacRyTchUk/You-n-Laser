...
var
  ...
  BlockCount, hspd, vspd, px, py, mxold, myold, mousex, mousey: int; 
  Player: spr;
  CurModeOfScreen: Str;
  ...
  StartScreenX,StartScreenY,CurScreenX,CurScreenY,ActiveItemX,ActiveItemY,activateitemID: int;
  CurScreen : int := 1;
...
begin
 ...
 // while not vSpase do begin Sleep(100); //TextOut(200, 200, booltostr(vSpase)); Write('n'); 
 // end;
 //Sleep(5000); 
  textboxtest.Destroy;
  textboxback.Destroy;
  ...
  case CurModeOfScreen of 
   ...
   //  if (mx < 100) and (my < 100) then begin CurModeOfScreen := 'Game' end else if (mx > Window.Width - 100) and (my > Window.Height - 100) then begin CurModeOfScreen := 'MiniGame'; end else vclick := false;
    ...
    //1if not (ObjectUnderPoint(mx,my) = playbutt) and not (ObjectUnderPoint(mx,my) = infobutt) then vclick := false;
     ...
     'MiniGame' : begin TextOut(mx,my,'Hi'); end;
  end; end;
...
var LaserPart := new spr(-6,-6,'Sprites\Effects\LaserParticle.png');
...
begin
  ...
  step := 2; sleeptime := 1;  squaresize := 6; otstx := 78 - squaresize div 2; otsty := 13 - squaresize div 2; 
...
begin
  if delornew = 1 then begin
    LockDrawingObjects; minigamefield[sx,sy].objectspr.Destroy; minigamefield[sx,sy].objectspr := minigameBlocks[minigamefield[sx,sy].rotate-1].Clone; 
          ...
          2:
            begin
              case currotation of 
                ...
                2: questrotating := 0;
                3: questrotating := 0;
                4: begin partlaserdraw(sx, sy, 2, 0); changePlate(sx,sy,1); partlaserdraw(sx, sy, 3, 1); changePlate(sx,sy,1); questrotating := 3; end;
              end; end;
          3:
            begin
              case currotation of 
...
var finishanim := new spr(nilxpos+xfin*64-64,nilxpos+xfin*64+64-1,64,'Sprites\Blocks\Mng.Finish.Aktive.png');
UnLockDrawingObjects;
Sleep(100);
...
//=====================Отрисовка лазера в мини игре========================
procedure alllaserdrawing(startx, starty: int);
begin
  var sx, sy, currotate: int;
  var stop : bool;
  sx := startx; sy := starty;
  currotate := minigamefield[startx, starty].rotate;
  partlaserdraw(startx,starty,currotate,1);
  while  true do
  begin
      ...
      case currotate of
        ...
        1:
          if (sy > 1) then begin
            ...
            if (minigamefield[sx, sy - 1].empty  <> false) then if rot = 0 then break else begin currotate := rot;  end;
        ...
        2: {if (sx < 16) and (minigamefield[sx+1,sy].typeofobgect  <> 'finish') then begin 
          ...
          if (sx < 16) then begin
            ...
            if (minigamefield[sx + 1, sy].empty  <> false) then if rot = 0 then exit else begin currotate := rot;  end;
        ...
        3: {if (sy < 8) and (minigamefield[sx,sy+1].typeofobgect  <> 'finish') then begin 
          ...
          if (sy < 8) then begin
            //if (minigamefield[sx,sy-1].typeofobgect  = 'finish') then TextOut(100,100,'ohyeah');
            var rot: int := questrotating(sx, sy + 1, currotate);
            if (minigamefield[sx, sy + 1].empty  <> false) then if rot = 0 then exit else begin currotate := rot;  end;
           // Writeln(questrotating(sx, sy + 1, currotate));
        ...
        4: {if (sx > 1) and (minigamefield[sx-1,sy].typeofobgect  <> 'finish') then begin 
          ...
          if (sx > 1) then begin
            ...
            Dec(sx);
            if (sx > 1) then begin
              if minigamefield[sx - 1, sy].typeofobgect  = 'finish' then begin finishanimation(sx-1,sy,currotate); 
              //
              Stop := false;end;
 ...
 // if stop = false then break;
  ...
  cursor.MoveTo(alldotsofliser.Item[alldotsofliser.Count-1].position.x-3,alldotsofliser.Item[alldotsofliser.Count-1].position.y+3);
end;
//end;
...
begin
  ...
  if (0  >= px  + hspd) then begin Textout(400, 20, 'left   ');  undrawfield(); Dec(CurScreenX);CurScreen := LocationOfFields[CurScreenx,CurScreenY]; loadscreenfrombuffer(FieldsName[CurScreen-1], CurScreen); drawfield(1, CurScreen);  Player.Moveto(window.Width - 70, py); end;
  if (px + Player.Width - 10 + hspd >= Window.Width) then begin TextOut(400, 20, 'right  '); 
  undrawfield(); 
  Inc(CurScreenX);
  CurScreen := LocationOfFields[CurScreenx,CurScreenY];
  loadscreenfrombuffer(FieldsName[CurScreen-1], CurScreen); drawfield(1, CurScreen);  Player.Moveto(8, py); end;
  if (0  >= py - vspd) then begin TextOut(400, 20, 'up    '); undrawfield(); Dec(CurScreenY); CurScreen := LocationOfFields[CurScreenx,CurScreenY]; loadscreenfrombuffer(FieldsName[CurScreen-1], CurScreen); drawfield(1, CurScreen);  Player.Moveto(px, Window.Height - Player.Height); end;
  if (py - vspd  >= Window.Height) then begin TextOut(400, 20, 'down '); undrawfield(); Inc(CurScreenY); CurScreen := LocationOfFields[CurScreenx,CurScreenY]; loadscreenfrombuffer(FieldsName[CurScreen-1], CurScreen); drawfield(1, CurScreen);  Player.Moveto(px, player.Height);  end;
  ...
  if (plxl < 0) or (plyu < 0) or (plxr > 20) or (plyd > 10) then begin exit; end else begin///!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    for i:= 24 to 28 do begin
  if (Fields[plxl, plyd, 4, CurScreen] = i) or (Fields[plxr, plyd, 4, CurScreen] = i) then platform := true;
  end;
      if 
      //((ObjectUnderPoint(plxl,plyu ) = blocksoffield[i,1]) or 
      //(ObjectUnderPoint(plxr,plyd) = blocksoffield[i,1]))  
      (Fields[plxl, plyd, 1, CurScreen] <> 0) or (Fields[plxr, plyd, 1, CurScreen] <> 0) or platform
      then 
      begin if not OnFloor then 
  ...
  if (plxl >= 0) and (plyu >= 0) and (plxr <= 20) and (plyd <= 10) then begin
    if  vLeft and ((Fields[plxl, plyd, 1, CurScreen] <> 0) or (Fields[plxl, plyu, 1, CurScreen] <> 0)) then begin 
  ...
  plxl := (plxl - (plxl mod 64)) div 64; plyu := (plyu - (plyu mod 64)) div 64; plxr := (plxr - (plxr mod 64)) div 64; plyd := (plyd - (plyd mod 64)) div 64;  
  if OnFloor and ((Fields[plxl, plyu, 5, CurScreen] <> 0) or (Fields[plxr, plyu, 5, CurScreen] <> 0)) then begin
  if (Fields[plxl, plyu, 5, CurScreen] <> 0) then begin  
  activateitemID := Fields[plxl, plyu, 5, CurScreen];
  activeitemx := plxl; activeitemy:= plyu; end 
  else 
  begin
  activateitemID := Fields[plxr, plyu, 5, CurScreen];
  ...
  if vactivateitem then begin vActivateItem := false;
  activateitemID := 0;
...
begin
  ...
  //for i := 2 to 17 do partlaserdraw(i,2,0);
  xp := 1; yp := 8;
  minigamefield[xp, yp].typeofobgect := 'start';
  minigamefield[xp, yp].rotate := 1;
  minigamefield[xp, yp].objectspr := minigameBlocks[8].Clone; 
  minigamefield[xp, yp].objectspr.MoveTo(nilxpos+xp*64-64,nilypos+yp*64-64); 
  minigamefield[xp, yp].objectspr.Visible := true;
  minigamefield[xp, yp].empty := true; 
  
  minigamefield[xp, yp - 7].typeofobgect := 'plate';
  minigamefield[xp, yp - 7].rotate := 2;
   minigamefield[xp, yp - 7].objectspr := minigameBlocks[minigamefield[xp, yp - 7].rotate-1+4].Clone; 
  minigamefield[xp, yp - 7].objectspr.MoveTo(nilxpos+xp*64-64,nilypos+(yp - 7)*64-64); 
  minigamefield[xp, yp - 7].objectspr.Visible := true;
  minigamefield[xp, yp - 7].empty := true;
  
  
  minigamefield[xp + 6, yp - 7].typeofobgect := 'plate';
  minigamefield[xp + 6, yp - 7].rotate := 3;
   minigamefield[xp + 6, yp - 7].objectspr := minigameBlocks[minigamefield[xp + 6, yp - 7].rotate-1+4].Clone; 
  minigamefield[xp + 6, yp - 7].objectspr.MoveTo(nilxpos+(xp + 6)*64-64,nilypos+(yp - 7)*64-64); 
  minigamefield[xp + 6, yp - 7].objectspr.Visible := true;
  minigamefield[xp + 6, yp - 7].empty := true;
  
  
  
  minigamefield[xp + 6, yp - 5].typeofobgect := 'plate';
  minigamefield[xp + 6, yp - 5].rotate := 1;
   minigamefield[xp + 6, yp - 5].objectspr := minigameBlocks[minigamefield[xp + 6, yp - 5].rotate-1+4].Clone; 
  minigamefield[xp + 6, yp - 5].objectspr.MoveTo(nilxpos+(xp + 6)*64-64,nilypos+(yp - 5)*64-64); 
  minigamefield[xp + 6, yp - 5].objectspr.Visible := true;
  minigamefield[xp + 6, yp - 5].empty := true;
  
  
  
  minigamefield[xp + 12, yp - 5].typeofobgect := 'plate';
  minigamefield[xp + 12, yp - 5].rotate := 3;
  minigamefield[xp + 12, yp - 5].objectspr := minigameBlocks[minigamefield[xp + 12, yp - 5].rotate-1+4].Clone; 
  minigamefield[xp + 12, yp - 5].objectspr.MoveTo(nilxpos+(xp + 12)*64-64,nilypos+(yp - 5)*64-64); 
  minigamefield[xp + 12, yp - 5].objectspr.Visible := true;
  minigamefield[xp + 12, yp - 5].empty := true;
  
  minigamefield[xp + 12, yp].typeofobgect := 'plate';
  minigamefield[xp + 12, yp].rotate := 4;
  minigamefield[xp + 12, yp ].objectspr := minigameBlocks[minigamefield[xp + 12, yp].rotate-1+4].Clone; 
  minigamefield[xp + 12, yp ].objectspr.MoveTo(nilxpos+(xp + 12)*64-64,nilypos+(yp)*64-64); 
  minigamefield[xp + 12, yp ].objectspr.Visible := true;
  minigamefield[xp + 12, yp].empty := true;
  
  
  minigamefield[xp + 1, yp].typeofobgect := 'plate';
  minigamefield[xp + 1, yp].rotate := 1;
  minigamefield[xp + 1, yp ].objectspr := minigameBlocks[minigamefield[xp + 1, yp].rotate-1+4].Clone; 
  minigamefield[xp + 1, yp ].objectspr.MoveTo(nilxpos+(xp + 1)*64-64,nilypos+yp*64-64); 
  minigamefield[xp + 1, yp ].objectspr.Visible := true;
  minigamefield[xp + 1, yp].empty := true;
  
  
  minigamefield[xp + 1, yp - 3].typeofobgect := 'finish';
  minigamefield[xp + 1, yp - 3 ].objectspr := minigameBlocks[12].Clone; 
  minigamefield[xp + 1, yp - 3 ].objectspr.MoveTo(nilxpos+(xp + 1)*64-64,nilypos+(yp - 3)*64-64); 
  minigamefield[xp + 1, yp - 3 ].objectspr.Visible := true;
  minigamefield[xp + 1, yp - 3].empty := true;
  
  
  alllaserdrawing(xp, yp);
...
begin
  ...
  ATimer.Start;
  CanMove := true;
  StartScreenX := 2; StartScreenY := 4;
  CurScreenx := 2; CurScreeny:=4;
  var startScreen:= LocationOfFields[StartScreenX,StartScreenY];
  CurScreen := StartScreen;
  //=========================Начало программы=====================================
 
  loadscreenfrombuffer(FieldsName[StartScreen-1], StartScreen);
