...
var cursor := new SquareABC(-8,-8,8,clwhite);
var alldotsofliser := new List<SquareABC>;
procedure partlaserdraw(xf,yf,rotate : int);
var i, xnul,ynul : int;
begin
if rotate = 1 then ynul := 1 else xnul := 1;
for i := 1 to 16 do begin
cursor.MoveTo(xf*64+4*i*xnul-2,yf*64+4*i*ynul-2);
SetPenColor(clred);  SetBrushColor(clRed);
alldotsofliser.Add(new SquareABC(xf*64+4*i*xnul,yf*64+4*i*ynul,4,clred));
SetPenColor(clWhite); SetBrushColor(clwhite);
Sleep(10);
end;
end;

procedure alllaserdrawing(strartx,starty : int);
begin
partlaserdraw(strartx,starty,1);
end;
//=====================Менюшка================================
procedure menu();
