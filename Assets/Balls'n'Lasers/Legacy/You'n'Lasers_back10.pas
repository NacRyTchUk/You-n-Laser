...
begin
  ...
  playbutt := new spr(20, 325, 'Sprites\UI\PlayButt2.png'); end;
              ...
              infobutt :begin infobutt := new spr(20, 450, 'Sprites\UI\InfoButt2.png'); end;
               ...
               fon.MoveTo(-60+((Window.Width div 2 - mousex)) div 40,-60+((Window.Height div 2 - mousey)) div 40);
    ...
    'Game':
      begin
        ...
        case key of
          ...
          VK_Space: begin if (plxl < 0) or (plyu < 0) or (plxr > 20) or (plyd > 10) or not CanMove then exit;  vSpase := true;  if not CanMove then exit; if canmove and OnFloor then begin vspd := vforce; if LookR then begin if not vRight then Player.State := 3; end else begin  if not vLeft then Player.State := 6; end; end; end;
...
begin
  ...
  if (px + Player.Width - 10 + hspd >= Window.Width) then begin
    ...
    if (CurScreen = 49) then begin housespr := new pic(400,7*64-200,'Sprites\Blocks\House.outside.png'); housespr.ToBack; end;
...
if listofachive.Count = 0 then exit;
SetFontName('Comic Sans MS');
...
begin
  ...
  playbutt.moveon(10,0);
  infobutt.moveon(10,0);
  exitbutt.moveon(10,0);
  playbutt2.moveon(10,0);
  infobutt2.moveon(10,0);
  exitbutt2.moveon(10,0);
  end;
  //Sleep(5000);
  //game();
  while true do 
