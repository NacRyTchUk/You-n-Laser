...
var
  MagicSphere : array of magsphere;
...
var curspherespr : array[1..10] of spr;
var cursphereid : array[1..10] of byte;
var curspherepos : array[1..10] of position;
...
begin
  ...
  //back.Scale(3);
  //back.ToBack; ///!!!!!!!!!!!!!!!!!!!!!!!
  for i:=1 to 10 do cursphereid[i] := 0;
  for i:=1 to 10 do cursphereid[i] := 0;
  curspherenumb := 0;
    ...
    for i := 0 to 10 do 
      ...
      begin
        ...
        begin
          ...
          if (layer = 5) and (Fields[j, i, layer] = 54)  then begin
            Inc(curspherenumb);
            curspherepos[curspherenumb].x := j;
            curspherepos[curspherenumb].y := i;
          ...
          curspherespr[curspherenumb] := Blocks[Fields[j, i, layer]].Clone; //Копируем спрайт из буфера
          curspherespr[curspherenumb].MoveTo(j * 64, I * 64); // Помещаем его на нужные координаты и делаем видимым
          curspherespr[curspherenumb].Visible := true;
          end else begin
...
begin
  ...
  for i:=1 to curspherenumb do curspherespr[i-1].Destroy;
...
begin
  ...
  finishanim.NextFrame;
  Sleep(1000);
  TextBoxStart(Window.Width div 2 - 300, Window.Height div 2-150, '1&конгратс');
  //
  finishanim.Destroy;
  cursor.MoveTo(-8, -8);
...
begin
 ...
 // for i:=1 to 4 do TextBoxStart(100,100,textforboxes[1,i]);
  ...
  if OnFloor and ((Fields[plxl, plyu, 5] <> 0) or (Fields[plxr, plyu, 5] <> 0)) then begin
    if not dataforsave.magicsphereaktive[cursphereid[curspherenumb]] then begin
    ...
    if not vActivateItem then begin for i:=1 to 1 do begin TextBoxStart(100,100,textforboxes[2,i]); end;  end;
    vActivateItem := true;
...
begin
 ...
 IsMiniGameIsFinished := false;
  ...
  //for i := 2 to 17 do partlaserdraw(i,2,0);
  xp := 1; yp := 8;
  loadminigamefrombuffer(FieldsMiniGame[cursphereid[curspherenumb]-1]);
  drawminigame;
  ...
  field.Destroy;
  cursor.MoveTo(-8, -8);
  dataforsave.magicsphereaktive[cursphereid[curspherenumb]] := true;
  vActivateItem := false;
